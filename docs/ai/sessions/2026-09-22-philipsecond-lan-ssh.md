...
On **PHILIPSECOND**, the helper installed Windows OpenSSH Server, restricted
the standard inbound rule to the Private profile and `LocalSubnet`, disabled
password login, and authorized PC-PHILIP-WINDOWS's committed public key for
administrator login. The service is Automatic and running, TCP 22 is listening,
and a loopback connection test passed. Its ED25519 server host fingerprint is:

```text
SHA256:RQLkviLRLJwIqLtdhO0S0+WoSy1oeoUN/uPlhmsiKYc
```

The first service starts failed for two independent reasons. Norton Behavioral
Protection terminated the signed Microsoft `sshd.exe`; the user added a narrow
exception for that executable. The host private keys newly generated by an
elevated interactive shell also retained an explicit permission for that user,
which prevented service-mode startup. Their original ACLs were saved under
`C:\ProgramData\ssh\dotrepo-backups`, then the keys were restricted to SYSTEM
and Administrators. The helper now applies that restricted ACL automatically
only to private host keys it creates, while preserving pre-existing keys and
their ACLs.
